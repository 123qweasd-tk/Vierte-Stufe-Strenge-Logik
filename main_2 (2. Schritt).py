...
def replace_n_times_fn(list_tetradic_level_raw, writer, *args):
    ...
    if list_tetradic_level_raw[0].count('u') != 0:
        ...
        first_formula = [both_new_formulas[:][0][:], list_tetradic_level_raw[:][1][:], [str(both_new_formulas[2])+'A']]

                
        writer.writerow(first_formula)
                
        second_formula = [both_new_formulas[:][1][:], list_tetradic_level_raw[:][1][:], [str(both_new_formulas[2])+'N']]
